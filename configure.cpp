...
void initialize_simulation() {
  ...
  global_clock.last_tick =
      (unsigned long)(24 * 60 * 60 * 1000) / global_clock.ms_per_tick;
  global_clock.hour = -1;
  ...
  for (auto &v : config["drivers"].GetArray()) {
    ...
    assertm(config[v.GetString()]["load"].HasMember("users"),
            "No hourly users load");
    assertm(config[v.GetString()]["load"].HasMember("content"),
            "No hourly content load");
    assertm(config[v.GetString()]["load"].HasMember("sites"),
            "No hourly site load");
    assertm(config[v.GetString()]["load"]["requests"].IsArray(),
            "load must be an array");
    assertm(config[v.GetString()]["load"]["users"].IsArray(),
            "load must be an array");
    assertm(config[v.GetString()]["load"]["content"].IsArray(),
            "load must be an array");
    assertm(config[v.GetString()]["load"]["sites"].IsArray(),
            "load must be an array");
    assertm(config[v.GetString()]["load"]["requests"].Size() == 24,
            "load must be an 24 array");
    assertm(config[v.GetString()]["load"]["users"].Size() == 24,
            "load must be an 24 array");
    assertm(config[v.GetString()]["load"]["content"].Size() == 24,
            "load must be an 24 array");
    ...
    for (int i = 0; i < 24; i++) {
      ...
      hour_load.requests =
          config[v.GetString()]["load"]["requests"][i].GetInt();
...
void configure(const char *node) {
  ...
  assertm(config[node].HasMember("cache"),
          "Node does have cache configuration");
  assertm(config[node].HasMember("network_latency"),
          "Node does have cache configuration");
  ...
  assertm(config[node].HasMember("growthmodel"),
          "Node does not specify a growth model for capacity");
  assertm(config[node].HasMember("limit"),
          "Node does not specify a limit on capacity");

  assertm(config[node]["cache"].IsObject(), "Cache needs to be an object");
  assertm(config[node]["network_latency"].IsObject(),
          "network latency needs to be an object");
  assertm(config[node]["self_time"].IsObject(),
          "self time needs to be an object");
  assertm(config[node]["dependencies"].IsObject(),
          " depedencies needs to be an object");
  ...
  if (strcmp(config[node]["growthmodel"].GetString(), "linear") == 0) {
    ...
    cerr << "Growth model unsupported "
         << config[node]["growthmodel"].GetString() << endl;
    assert(false);
  }
  ...
  if (strcmp(config[node]["balanced"].GetString(), "requests") == 0) {
    ...
    cerr << "Load balancing model unsupported "
         << config[node]["growthmodel"].GetString() << endl;
    assert(false);
  }
...
bool parse_config(char *file) {
  ...
  if (config.HasParseError()) {
    cerr << "Error Parsing " << file << " :"
         ...
         << " at line: " << csw.GetLine() << " col: " << csw.GetColumn()
         << endl;
    return false;
  }
  assertm(config.IsObject(), "Input file is not an object");
  assertm(config.HasMember("drivers"),
          "Need at least one external load driver");
  assertm(config["drivers"].IsArray(),
          "Set of load drivers needs to be an array");
  for (auto &v : config["drivers"].GetArray())
